...
#define PRIVATE_APLUGINSDK_REGISTER_FEATURE(returnType, featureGroup, featureName, ...)                                \
    APLUGINSDK_NO_EXPORT const char* private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_returnType = #returnType; \
    APLUGINSDK_NO_EXPORT const char* private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_parameters = #__VA_ARGS__; \
    APLUGINSDK_NO_EXPORT returnType private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_function(__VA_ARGS__)
...
#define PRIVATE_APLUGINSDK_RECORD_FEATURE(featureGroup, featureName)                                                   \
    private_APluginSDK_registerFeature(#featureGroup, #featureName,                                                    \
        private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_returnType,                           \
        private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_parameters,                           \
        (void*) private_APluginSDK_plugin_implemenation_##featureGroup##_##featureName##_function);

/* private plugin class macros */
#define PRIVATE_APLUGINSDK_REGISTER_CLASS(interfaceName, className)                                                    \
    "Classes are not supported by C and therefore not by the C API, too!"
